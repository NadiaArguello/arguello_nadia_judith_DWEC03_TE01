...
function crearTablero(localizacion) {
    ...
    return Array(tamanoTablero)
        .fill(null) // Inicializa todas las posiciones del array con null
        .map(() => Array(tamanoTablero).fill(0)); //Rellena cada posición del array con otro array del mismo tamaño con valor 0 en sus celdas. De esta forma crea un array bidimensional del tamaño deseado
}
    ...
    tamanosBarco.forEach(tamanoBarco => {
        ...
        while (!colocado) {
            ...
            if (puedeColocarseBarco(tablero, x, y, tamanoBarco, direccion)) {
                ...
                estadoBarco[idBarcoActual] = tamanoBarco; // El estado del barco al inicio de la partida es el tamaño del barco. Según vayan bombardeándolo, se irá restando -1 al estado hasta que sea 0. En ese momento el barco estará hundido
                idBarcoActual++;
                colocado = true;
            }
        }
